...
// src/chatgpt/index.ts
var { HttpsProxyAgent } = import_https_proxy_agent.default;
dotenv.config();
...
function setupProxy(options) {
  const httpsProxy = process.env.HTTPS_PROXY || process.env.ALL_PROXY;
  const socksProxy = process.env.SOCKS_PROXY_HOST && process.env.SOCKS_PROXY_PORT ? `${process.env.SOCKS_PROXY_HOST}:${process.env.SOCKS_PROXY_PORT}` : null;
  if (httpsProxy) {
    options.fetch = (input, init) => {
      const agent = new HttpsProxyAgent(httpsProxy);
      return (0, import_node_fetch.default)(input, { ...init, agent });
    };
  } else if (socksProxy) {
    const agent = new import_socks_proxy_agent.SocksProxyAgent(socksProxy);
    options.fetch = (input, init) => {
      return (0, import_node_fetch.default)(input, { ...init, agent });
    };
  }
